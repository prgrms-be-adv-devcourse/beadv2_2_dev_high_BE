package com.dev_high.deposit.infrastructure.kafka;
...
public class PaymentEventListener {
    ...
    public void handleDepositPaymentComplete(KafkaEventEnvelope<Map<String, Object>> envelope, ConsumerRecord<?, ?> record) {
        ...
        if (id == null || id.isBlank()) {
            log.warn("필수 파라미터 누락 - userId: {}, Offset: {}",
                    id, record.offset());
        }

        DepositOrderDto.CompleteCommand command = DepositOrderDto.CompleteCommand.of(id);
        try {
            depositOrderService.completeOrder(command);
        } catch (Exception e) {
    ...
    public void handleDepositPaymentFail(KafkaEventEnvelope<Map<String, Object>> envelope, ConsumerRecord<?, ?> record) {
        ...
        DepositOrderDto.FailCommand command = DepositOrderDto.FailCommand.of(id);
        try {
            depositOrderService.failOrder(command);
        } catch (Exception e) {
            log.error("예치금 주문 완료 처리 실패 id: {}", id, e );
        }
    }
