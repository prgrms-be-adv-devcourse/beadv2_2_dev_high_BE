...
public class AuctionSearchService {
    ...
    public ApiResponseDto<Page<AuctionDocument>> searchAuctions(String keyword,
        ...
        if(minStartPrice != null || maxStartPrice != null) {
            boolQuery.filter(f -> f.range(r -> r.number(n -> {
                ...
                if (minStartPrice != null) {
                    n.gte(minStartPrice.doubleValue());
                }
                if (maxStartPrice != null) {
                    n.lte(maxStartPrice.doubleValue());
                }
                return n;
            })));
        }
        ...
        if(startFrom != null || startTo != null) {
            boolQuery.must(q -> q.range(r -> r.date(d -> {
                d.field("auctionStartAt");
                if (startFrom != null) {
                    d.gte(startFrom.toString());
                }
                if (startTo != null) {
                    d.lte(startTo.toString());
                }
                return d;
            })));
        }
